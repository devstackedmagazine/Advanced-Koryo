...
export function VehicleCard({ v, variant = "vehicle" }: { v: VehicleRow; variant?: "vehicle" | "auction" }) {
  ...
  const cardTitle = (lang === "ar" ? v.title_ar : v.title_en) || `${v.make} ${v.model}`;

  return (
    <div className="group relative rounded-2xl bg-card border border-border hover:border-gold hover:shadow-card transition-all overflow-hidden flex flex-col cursor-pointer">
      ...
      <div className="p-4 lg:p-5 flex-1 flex flex-col">
        ...
        {/* Action buttons sit ABOVE the stretched link (z-[2]) so they stay independently clickable */}
        <div className="relative z-[2] mt-4 pt-3 border-t border-border flex items-center gap-2">
          <a
            href={wa}
            target="_blank"
            rel="noopener"
